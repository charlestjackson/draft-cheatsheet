...
if (Meteor.is_client) {
  ...
  Template.player.events = {
    ...
    /*'dragstart .player' : function(e) {
      console.log($(e.target).position()); 
    },
    'dragstop .player' : function(e) {
      ...
      var $prevSiblings = $target.prevAll();
      for (var $sib in $prevSiblings) {
         var sibpos = $sib.position();
	 if (pos.top < sibpos.top) {
           alert("moved above " + $sib);
	 }
      }
    }*/
  }
...
$(function() {
  ...
  function checkUp($target) {
    ...
    if (newOverall != startingPos) {
      ...
      Players.update( { name : targetPName }, { $set : { overall_pos : newOverall } }, { multi : true} );
    }
  }
